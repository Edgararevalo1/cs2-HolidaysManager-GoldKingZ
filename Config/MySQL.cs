...
public class MySqlDataManager
{
    ...
    private static readonly Dictionary<string, string> CurrentTableSchema = new Dictionary<string, string>
    {
        ...
        {"Toggle_Snow", "INT NOT NULL DEFAULT 0"},
        {"Toggle_Props", "INT NOT NULL DEFAULT 0"},
    ...
    private static List<MySqlConnectionStringBuilder> GetConnectionStrings()
    {
        ...
        foreach (var serverConfig in config.MySql_Servers)
        {
            try
            {
                ...
                Helper.DebugMessage($"Configured MySQL server: {serverConfig.Server}:{serverConfig.Port} with database: {serverConfig.Database}");
            }
            catch (Exception ex)
            {
                Helper.DebugMessage($"Error configuring MySQL server {serverConfig.Server}:{serverConfig.Port}: {ex.Message}", true);
            }
        }
    ...
    public static async Task<MySqlConnection?> GetConnectionAsync()
    {
        ...
        {
            Helper.DebugMessage("No MySQL servers configured");
            return null;
        }
        ...
        for (int attempt = 0; attempt < Configs.Instance.MySql_RetryAttempts; attempt++)
        {
            if (attempt > 0)
            {
                Helper.DebugMessage($"Retry attempt {attempt + 1} of {Configs.Instance.MySql_RetryAttempts}");
            }
            ...
            foreach (var connectionStringBuilder in connectionStrings)
            {
                try
                {
                    ...
                    Helper.DebugMessage($"Successfully connected to MySQL server: {connectionStringBuilder.Server}:{connectionStringBuilder.Port}");
                    return connection;
                }
                catch (OperationCanceledException ex)
                {
                    lastException = ex;
                    Helper.DebugMessage($"Connection timeout to {connectionStringBuilder.Server}:{connectionStringBuilder.Port}", true);
                }
                catch (MySqlException ex)
                {
                    lastException = ex;
                    Helper.DebugMessage($"MySQL error connecting to {connectionStringBuilder.Server}:{connectionStringBuilder.Port} - {ex.Message}", true);
                }
                catch (Exception ex)
                {
                    lastException = ex;
                    Helper.DebugMessage($"Failed to connect to {connectionStringBuilder.Server}:{connectionStringBuilder.Port} - {ex.Message}", true);
                }
            }

            if (attempt < Configs.Instance.MySql_RetryAttempts - 1)
            {
                Helper.DebugMessage($"Waiting {Configs.Instance.MySql_RetryDelay}s before next retry...");
                await Task.Delay(TimeSpan.FromSeconds(Configs.Instance.MySql_RetryDelay));
            }
        }

        Helper.DebugMessage($"Failed to connect to any MySQL server after {Configs.Instance.MySql_RetryAttempts} attempts", true);
        return null;
    }
    ...
    public static async Task<bool> CreateTableIfNotExistsAsync()
    {
        ...
        try
        {
            ...
            if (connection == null)
            {
                Helper.DebugMessage("Cannot create table - no MySQL connection available", true);
                return false;
            }
            ...
            if (!tableExists)
            {
                ...
                Helper.DebugMessage("Database table created successfully with all columns");
                return true;
            }
            else
            {
                await CheckAndUpdateTableSchemaAsync(connection);
                Helper.DebugMessage("Database table schema verified and updated if needed");
                return true;
            }
        }
        catch (Exception ex)
        {
            Helper.DebugMessage($"DB Init Error: {ex.Message}", true);
            return false;
        }
        finally
        {
    ...
    private static async Task CheckAndUpdateTableSchemaAsync(MySqlConnection connection)
    {
        try
        {
            ...
            foreach (var requiredColumn in CurrentTableSchema)
            {
                ...
                {
                    Helper.DebugMessage($"Adding missing column: {requiredColumn.Key}");
        ...
        {
            Helper.DebugMessage($"Error checking/updating table schema: {ex.Message}", true);
        }
    }
    ...
    private static async Task AddColumnToTableAsync(MySqlConnection connection, string columnName, string columnDefinition)
    {
        try
        {
            ...
            string alterTableQuery = $"ALTER TABLE CnD_PersonData ADD COLUMN {columnName} {columnDefinition} {afterColumn}";
            
            await using var command = new MySqlCommand(alterTableQuery, connection);
            await command.ExecuteNonQueryAsync();
            
            Helper.DebugMessage($"Successfully added column: {columnName}");
        }
        catch (Exception ex)
        {
            Helper.DebugMessage($"Error adding column {columnName}: {ex.Message}", true);
        }
    }
    ...
    public static async Task<bool> SaveToMySqlAsync(Globals_Static.PersonData data)
    {
        const string insertOrUpdateQuery = @"
            INSERT INTO CnD_PersonData 
                (PlayerSteamID, Toggle_Snow, Toggle_Props, DateAndTime)
            VALUES 
                (@PlayerSteamID, @Toggle_Snow, @Toggle_Props, @DateAndTime)
            ON DUPLICATE KEY UPDATE 
                Toggle_Snow = VALUES(Toggle_Snow),
                Toggle_Props = VALUES(Toggle_Props),
        ...
        try
        {
            ...
            if (connection == null)
            {
                Helper.DebugMessage("Cannot save data - no MySQL connection available", true);
                return false;
            }
        ...
        {
            Helper.DebugMessage($"Saving Values In MySql Error: {ex.Message}", true);
            return false;
        }
    }
    ...
    public static async Task<Globals_Static.PersonData> RetrievePersonDataByIdAsync(ulong steamId)
    {
        ...
        try
        {
            ...
            if (connection == null)
            {
                Helper.DebugMessage("Cannot retrieve data - no MySQL connection available", true);
            ...
            if (await reader.ReadAsync())
            {
                return new Globals_Static.PersonData
                {
                    ...
                    Toggle_Snow = reader.GetInt32("Toggle_Snow"),
                    Toggle_Props = reader.GetInt32("Toggle_Props"),
        ...
        {
            Helper.DebugMessage($"Retrieve Values In MySql Error: {ex.Message}", true);
        }
        return new Globals_Static.PersonData();
    }
    ...
    public static async Task<bool> DeleteOldPlayersAsync()
    {
        ...
        {
            Helper.DebugMessage($"Delete Old Players In MySql Error: {ex.Message}", true);
            return false;
        }
    }
    ...
    private static void AddPersonDataParameters(MySqlCommand command, Globals_Static.PersonData data)
    {
        ...
        command.Parameters.Add("@Toggle_Snow", MySqlDbType.Int32).Value = data.Toggle_Snow;
        command.Parameters.Add("@Toggle_Props", MySqlDbType.Int32).Value = data.Toggle_Props;
